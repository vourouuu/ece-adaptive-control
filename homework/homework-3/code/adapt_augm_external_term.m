...
%% Unknown external term
Theta_hat_nom = 1/Km;
Theta_real = [0 ; -1/J_real];
...
for i = 1:length(time_vec)
    ...
    Fx = Kx_hat_nom;
    Fr = Kr_hat_nom;
    u_nom = Fx * x + Fr * theta_c + Theta_hat_nom * T_L;
    u = u_nom + Dx_hat * x + Dr_hat * theta_c + Di_hat * z + DT_hat * T_L;
    
    % 3. Euler integration
    z_dot = theta_c - x(1); % z_dot = θc - θ
    z = z + z_dot * dt;

    x_dot = A_real * x + B_real * u + Theta_real * T_L;
    x = x + x_dot * dt;
...
saveas(f1, 'tracking-5-uncertainty-T.png');
...
plot(time_vec, DT, 'm');
xlabel('Time (sec)');
legend({'$\hat{D}_T$'}, 'Interpreter', 'latex');

saveas(f2, 'gains-5-uncertainty-T.png');